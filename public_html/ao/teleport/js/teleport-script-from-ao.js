(function() {
    if(window.AO === undefined) {
        AO = {
            ...
            displayNativeForSecuredComm : false
        };
    }
})();
...
$(document).ready(function(){      
    ...
    if (AO.isAndroid || AO.isIOS)        
    {
        if ( window.location.origin.indexOf("secured-comm") > -1 && !AO.displayNativeForSecuredComm) {
            ...
            instructions = '<p>Or go to the <a class="ao-anchor-link btn btn-secondary" href="#" role="button">mobile site &raquo;</a></p>'        
        }        
    }
    ...
    var showNativeSetup = function() {
        ...
        if (AO.isIOS) {
            ...
            if (AO.isChatFOSS) {
                $('meta[name=apple-itunes-app]').attr('content', 'app-id=1005628468, app-argument=' + url);              
            } else if (AO.isPersonal) {
                $('meta[name=apple-itunes-app]').attr('content', 'app-id=1042907503, app-argument=' + url);                
            } else {
                $('meta[name=apple-itunes-app]').attr('content', 'app-id=1005595620, app-argument=' + url);                
            }
        }        
    }
    ...
    */
    ...
    var processLaunch = function(isOpenBrowserVersion) {           
        ...
        if (AO.isAndroid) {
            ...
            if (AO.isKindle) {
                androidScheme += "//os=kindle"; //"Kindle";
            }
            if (AO.isChatFOSS) {
